...
class MessageHandler:
    ...
    async def process_message(self, body, from_number):
        ...
        if state == 'initial':
            ...
            if body.strip() == '1':
                ...
                send_message(from_number, "Please reply with '1' for a survey, '2' for conversation")
    ...
    async def send_welcome_message(self, from_number, is_returning_user):
        if is_returning_user:
            welcome_message = (
                "Welcome back to Blocky! We're glad to see you again. We've noted your request to connect with [Provider Name]. "
                "Let's ask a few questions to better understand your needs and customize your blockchain contract.\n\n"
                "Before we dive into the technical aspects, we'll start with a couple of lighter questions to assess your perspective and tailor our approach accordingly. If you're ready to continue, type 'yes'."
            )
        else:
            welcome_message = (
                "Welcome to Blocky! We've noted your request to connect with [Provider Name]. "
                "Let's ask a few questions to better understand your needs and customize your blockchain contract.\n\n"
                "Before we dive into the technical aspects, we'll start with a couple of lighter questions to assess your perspective and tailor our approach accordingly. If you're ready to continue, type 'yes'."
            )
    ...
    async def start_survey(self, from_number):
        survey_data = self.db.get_document('surveys_LAHACKS', 'community_survey_LAHACKS')
        if survey_data:
            ...
            if questions:
                ...
                if 'surveys_LAHACKS' not in current_survey or 'responses' not in current_survey['surveys_LAHACKS']:
                    self.db.update_document('whatsapp_conversations', from_number, {'surveys_LAHACKS': {'current_question': 0, 'responses': []}})
    ...
    async def handle_survey(self, body, from_number, user_doc):
        survey_state = user_doc.get('surveys_LAHACKS', {})
        current_question_index = survey_state.get('current_question', 0)
        survey_ref = self.db.get_document('surveys_LAHACKS', 'community_survey_LAHACKS')
        if survey_ref:
